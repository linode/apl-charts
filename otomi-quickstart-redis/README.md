...
2. Optional, but recommended: Create a (sealed) secret that you share between the Redis cluster and the workloads using it. Add this to `auth.existingSecret` (secret name) and `auth.existingSecretPasswordKey` (secret key). This prevents that the autogenerated secret from Redis gets cycles, while the application runs with a stale credential.
3. Optional: Change the `replicaCount`. Default is set to `1`.
4. Optional: Configure the Prometheus `ServiceMonitor` to monitor the Redis cluster using the team's Prometheus:
