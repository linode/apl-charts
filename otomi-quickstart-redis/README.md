...
1. Go to the `values` tab and fill in a name for your Redis cluster
2. Optional: Change the `replicaCount`. Default is set to `1`.
3. Optional: Configure the Prometheus `ServiceMonitor` to monitor the Redis cluster using the team's Prometheus:

```yaml
metrics.enabled: true
metrics.serviceMonitor.enabled: true
metrics.serviceMonitor.additionalLabels:
  prometheus: team-<team-name>
```

4. Use a `secretKeyRef` to configure the environment variables for the container to use the Redis cluster:
...
- name: REDIS_PASSWORD
  valueFrom:
    secretKeyRef:
      ...
      key: redis-password
```
4. Optional: Turn off authentication by setting `auth.enabled=false`
...
| `replica.replicaCount`                                      | Number of Redis&reg; replicas to deploy                                                                                                                                                                                           | `1`                      |
