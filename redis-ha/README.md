# Quick start for creating a Redis cluster (single instance or high availability)

The `redis-ha` Helm chart can be used to create a Redis cluster that automatically scales from a single instance to a high availability cluster with master-replica topology and HAProxy load balancer. By default this chart installs 1 Redis pod for development, but automatically enables HA features (HAProxy, NetworkPolicy) when replicas are increased to 2 or more.
...
3. **Configure deployment mode**:
   - **Single Instance (default)**: Keep `replicas: 1` for development/testing. HAProxy and NetworkPolicy are disabled automatically.
   - **High Availability**: Set `replicas: 2` or higher (recommended: `3`) for production. HAProxy and NetworkPolicy are enabled automatically.
4. Optional: Override automatic behavior:
   - Force HAProxy always on: `haproxy.enabled: true`
   - Force NetworkPolicy always on: `networkPolicy.enabled: true`
...
This chart supports two deployment modes based on the `replicas` setting:

#### Single Instance Mode (replicas: 1)
- **Use case**: Development, testing, or non-critical workloads
- **Components**: 1 Redis pod with Sentinel
- **Features**:
  - Persistent storage enabled by default
  - HAProxy automatically disabled (no need for load balancing)
  - NetworkPolicy automatically disabled
  - `min-replicas-to-write: 0` (allows writes with no replicas)

#### High Availability Mode (replicas: 2+)
- **Use case**: Production workloads requiring high availability
- **Recommended**: `replicas: 3` for optimal fault tolerance
- **Components**:
  - Multiple Redis pods (master-replica topology)
  - HAProxy load balancer (automatically enabled, scales up to min(replicas, 3))
  - NetworkPolicy (automatically enabled for secure pod-to-pod communication)
- **Features**:
  - **Redis Master-Replica Topology**: Multiple Redis instances with automatic master election
  - **Redis Sentinel**: Monitors Redis instances and handles automatic failover
  - **HAProxy Load Balancer**: Provides connection routing to current master and read/write splitting
  - **Network Policies**: Secure pod-to-pod communication with Istio service mesh integration
...
### Automatic HA Feature Management

The chart intelligently manages HA features based on your replica configuration:

| Feature | Single Instance (replicas: 1) | High Availability (replicas: 2+) |
|---------|-------------------------------|-----------------------------------|
| HAProxy | Disabled (not needed) | **Automatically enabled** |
| HAProxy Replicas | 1 | **min(replicas, 3)** |
| NetworkPolicy | Disabled | **Automatically enabled** |
| Istio Egress | N/A | **Always allowed** (for service mesh) |
| min-replicas-to-write | 0 (allow writes) | 0 (configurable to 1+ for stricter requirements) |

**Override behavior**: Set `haproxy.enabled: true` or `networkPolicy.enabled: true` to force-enable features regardless of replica count.

#### Migrating from Single Instance to HA

To upgrade from development to production:

1. Update `replicas` from `1` to `3` (or desired count)
2. HAProxy and NetworkPolicy are automatically enabled
3. Applications should connect to the HAProxy service endpoint (same name)
4. Sentinel automatically handles master election
5. No manual configuration changes needed

#### Network Policy and Istio Integration

When NetworkPolicy is enabled (automatically in HA mode or manually):
- Inter-Redis pod communication is secured
- Egress to Istio control plane (`istio-system` namespace) is automatically allowed for service mesh integration
- DNS resolution is permitted (UDP/TCP port 53 to all destinations and cloud provider DNS at 169.254.0.0/16)
- Custom ingress/egress rules can be added via `networkPolicy.ingressRules` and `networkPolicy.egressRules`
...
| `networkPolicy.enabled` | whether NetworkPolicy for Redis StatefulSets should be created. When `false`: NetworkPolicy is automatically enabled when `replicas > 1` (for secure pod-to-pod communication). When `true`: NetworkPolicy is always enabled regardless of replica count. when enabled, inter-Redis connectivity is created | bool | `false` |
...
| `redis.config.min-replicas-to-write` | Minimum number of replicas required before accepting writes. Set to `0` by default to allow writes in single instance mode | int | `0` |
...
| `replicas` | Number of redis master/slave. Set to `1` for single instance mode (dev/test), or `2+` for HA mode (recommended: `3` for production). HAProxy and NetworkPolicy are automatically enabled when `replicas > 1` | int | `1` |
...
| `haproxy.enabled` | Enabled HAProxy LoadBalancing/Proxy. When `false`: HAProxy is automatically enabled when `replicas > 1` (for HA setups). When `true`: HAProxy is always enabled regardless of replica count | bool | `false` |
...
| `haproxy.replicas` | Number of HAProxy instances. In HA mode, this is automatically set to `min(replicas, 3)` for optimal load distribution | int | `1` |
